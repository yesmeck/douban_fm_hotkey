  ...
  if (message.cmd == 'info') {
    ...
    if (message.cmd == 'pause' || message.cmd == 'love') {
      ...
      if (message.cmd == 'love') {
        ...
        notice(songInfo, 'http://localhost:1988/' + img);
      }
    }
  }
...
var notice = function(songInfo, image) {
  image = image || songInfo.song.coverUrl;
  if (window.webkitNotifications.checkPermission() == 0) {
    var notification = window.webkitNotifications.createNotification(
      image, songInfo.song.artistName, songInfo.song.songName
    );
